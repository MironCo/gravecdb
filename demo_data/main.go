...
func main() {
	...
	conn, err := client.Connect(fmt.Sprintf("http://localhost:%s", port))
	if err != nil {
	...
	if len(result.Rows) > 0 {
		if total, ok := result.Rows[0]["total"].(float64); ok && total > 0 {
			fmt.Printf("Database already has %.0f Person nodes — skipping seed.\n", total)
			return
		}
	}
	...
	fmt.Println("  2023-01 — TechCorp founded...")
	...
	fmt.Println("  2023-03 — Skills tracked, first project...")
	...
	fmt.Println("  2023-06 — David and Eve join...")
	...
	fmt.Println("  2023-09 — BigCo and CoolStartup enter...")
	...
	run("Frank-Grace friends", `MATCH (f:Person {name: "Frank"}), (g:Person {name: "Grace"}) CREATE (f)-[:FRIENDS_WITH {since: 2023}]->(g) AT TIME 1693699200`)

	// ---- CSV Import: Cities and Offices ----
	fmt.Println("  Loading cities and offices from CSV...")
	run("Load cities", `LOAD CSV WITH HEADERS FROM 'demo_data/cities.csv' AS row CREATE (c:City {name: row.name, country: row.country, population: row.population}) AT TIME 1693526400`)
	run("Load offices", `LOAD CSV WITH HEADERS FROM 'demo_data/offices.csv' AS row CREATE (o:Office {company: row.company, city: row.city, opened: row.opened}) AT TIME 1693526400`)

	// Link offices to companies and cities
	run("TechCorp SF office", `MATCH (c:Company {name: "TechCorp"}), (o:Office {company: "TechCorp", city: "San Francisco"}) CREATE (c)-[:HAS_OFFICE]->(o) AT TIME 1693526400`)
	run("TechCorp Austin office", `MATCH (c:Company {name: "TechCorp"}), (o:Office {company: "TechCorp", city: "Austin"}) CREATE (c)-[:HAS_OFFICE]->(o) AT TIME 1705276800`)
	run("CoolStartup Berlin office", `MATCH (c:Company {name: "CoolStartup"}), (o:Office {company: "CoolStartup", city: "Berlin"}) CREATE (c)-[:HAS_OFFICE]->(o) AT TIME 1693526400`)
	run("BigCo London office", `MATCH (c:Company {name: "BigCo"}), (o:Office {company: "BigCo", city: "London"}) CREATE (c)-[:HAS_OFFICE]->(o) AT TIME 1693526400`)
	run("BigCo Tokyo office", `MATCH (c:Company {name: "BigCo"}), (o:Office {company: "BigCo", city: "Tokyo"}) CREATE (c)-[:HAS_OFFICE]->(o) AT TIME 1693526400`)

	// Link offices to cities
	run("SF office->city", `MATCH (o:Office {city: "San Francisco"}), (c:City {name: "San Francisco"}) CREATE (o)-[:LOCATED_IN]->(c) AT TIME 1693526400`)
	run("Austin office->city", `MATCH (o:Office {city: "Austin"}), (c:City {name: "Austin"}) CREATE (o)-[:LOCATED_IN]->(c) AT TIME 1705276800`)
	run("Berlin office->city", `MATCH (o:Office {city: "Berlin"}), (c:City {name: "Berlin"}) CREATE (o)-[:LOCATED_IN]->(c) AT TIME 1693526400`)
	run("London office->city", `MATCH (o:Office {city: "London"}), (c:City {name: "London"}) CREATE (o)-[:LOCATED_IN]->(c) AT TIME 1693526400`)
	run("Tokyo office->city", `MATCH (o:Office {city: "Tokyo"}), (c:City {name: "Tokyo"}) CREATE (o)-[:LOCATED_IN]->(c) AT TIME 1693526400`)

	// ---- Jan 2024: Reorg — Alice promoted, new hires ----
	fmt.Println("  2024-01 — Reorg and promotions...")
	...
	fmt.Println("  2024-04 — Bob and David move to CoolStartup...")
	...
	fmt.Println("  2024-07 — New projects and collaborations...")
	...
	fmt.Println("  2024-10 — Maya joins, mentorship grows...")
	...
	// ---- Jan 2025: Alice staff promo, Eve promoted ----
	fmt.Println("  2025-01 — Alice promoted to Staff, Eve to Director...")
	...
	if err == nil {
		for _, row := range result.Rows {
			fmt.Printf("  %v: %v employees\n", row["c.name"], row["headcount"])
		}
	}

	result, err = conn.Query(`MATCH (c:City) RETURN COUNT(c) AS total`)
	if err == nil && len(result.Rows) > 0 {
		fmt.Printf("  Cities: %v\n", result.Rows[0]["total"])
	}

	result, err = conn.Query(`MATCH (o:Office) RETURN COUNT(o) AS total`)
	if err == nil && len(result.Rows) > 0 {
		fmt.Printf("  Offices: %v\n", result.Rows[0]["total"])
	}

	fmt.Println("")
	fmt.Println("  Timeline spans Jan 2023 → Jun 2025")
	fmt.Println("  Use the time slider to explore how the org evolved!")
	fmt.Println("")
